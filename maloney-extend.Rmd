...
In addition to the features above, dplyr can also be used to subset data using the filter function.

```{r}
chocolate <- filter(df, chocolate == 1)
as_tibble(head(chocolate))
```

This is a quick and easy way to subset data that will be of great use in many projects.