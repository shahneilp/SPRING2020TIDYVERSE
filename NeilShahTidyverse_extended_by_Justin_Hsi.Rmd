...
Note: %>% is actually part of the magrittr package but it's loaded with dplyr. 
...
## Using purrr:: functional programming with map functions

One final area to know about the tidyverse is the purrr package which enhances the tidyverse's functional programming abilities. It does this by providing a consistent and complete set of tools for working with functions and vectors. The main use of functional programming is to replace the use of for loops and make code easier to read and more succinct. Imagine that you have to iterate over every column of your dataframe and calculate a mean. And then you need to do it again for mode. And again and again for median, max, and min. Purrr can help you out and reduce the number of functions/for-loops you'd end up writing.

### Description:

map() (and its various cousins) will produce vectors of your desired types, while doing the specific operations on the columns for you.
...
### Example: Calculate the mean and median of each column

```{r}
# Let's ignore the non-numeric column "competitor"
map(as_tibble(df[,2:ncol(df)]), mean)
# Piping works as well
df[,2:ncol(df)] %>% map(median)
```


## Intermediary Conclusions

As you can see the tidyverse has a myriad of great tools to quickly analyze data and find insight--and most of this was possible due to the data already being in tidy form! We were able to quickly do a linear regression on a sorted, selected and now labeled group of data with one line of code; had it been a messy data set, most of our time would be devoted to cleaning! And we are able to quickly iterate and create new variables with the help of the tidyverse as well!
